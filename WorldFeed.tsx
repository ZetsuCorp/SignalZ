...
import TrendingCarousel from "./trending/TrendingCarousel"; // ✅ Trending component
...
export default function WorldFeed({ wallType }) {
  ...
  const [trending, setTrending] = useState([]); // ✅ Trending state
  ...
  return (
    <>
      {/* ✅ Trending Carousel sits above full layout */}
                  ...
                  {safeTags.includes("link") && post.caption?.startsWith("http") && (
                    isYouTubeOrTikTok(post.caption) && getEmbedUrl(post.caption) ? (
                      ...
                      >
                        ...
                        <div style={{ fontSize: "0.8rem", color: "#ccc" }}>{extractDomain(post.caption)}</div>
                      </a>
                    )
                  )}
